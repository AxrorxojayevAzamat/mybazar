<form action="?" class="big-filter-without-title-checkbox">
    ...
    @if ($brands->isNotEmpty())
        ...
        <div id="filter2" class="panel">
            @foreach($brands as $i => $brand)
                <div  class="custom-control custom-checkbox">
                    <input type="checkbox" name="brands" class="custom-control-input" id="brands-checkbox"
                           value="{{ $brand->id }}" data-id="{{ $brand->id }}" data-index="{{ $i }}">
                    <label class="custom-control-label" for="brands-checkbox">{{ $brand->name }}</label>
                </div>
            @endforeach
